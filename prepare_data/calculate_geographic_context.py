...
for line in tqdm(
    open(os.path.join(resource_folder, "hz_aoi.txt")), desc="reading aoi info"
):
    line = line.strip("\n")
...
for line in tqdm(
    open(os.path.join(resource_folder, "hz_roads.txt")),
    desc="reading roads info",
):
    line = line.strip("\n")
    osm_id, name, bbox = line.split("\t")
...
pair_idx = 0
for line in tqdm(
    open(os.path.join(resource_folder, "text_location_pair.demo")),
    desc="reading pairs info",
):
    line = line.strip("\n")
    items = line.split("")
    idx = str(pair_idx)
...
def update_utt(idx, osmid, rel_type):
    ...
    utt[idx][3].append(
        [
            discrete_num(total_min_x, scale_x, lx1),
    ...
    utt[idx][4].append(
        [
            sign(x - lx1) * min(20, int(abs(x - lx1) / abs(lx1 - lx2) * 10))
            + 23,
            sign(y - ly1) * min(20, int(abs(y - ly1) / abs(ly1 - ly2) * 10))
            + 23,
            sign(x - lx2) * min(20, int(abs(x - lx2) / abs(lx1 - lx2) * 10))
            + 23,
            sign(y - ly2) * min(20, int(abs(y - ly1) / abs(ly1 - ly2) * 10))
            + 23,
        ]
    )
...
inaoi = set()
for line in tqdm(
    open(os.path.join(resource_folder, "location_in_aoi.demo")),
    desc="generate data",
):
    line = line.strip("\n")
...
for line in tqdm(
    open(os.path.join(resource_folder, "location_near_aoi.demo")),
    desc="generate data",
):
    line = line.strip("\n")
    idx, osmid = line.split("\t")
    if idx + osmid in inaoi:
        continue
    update_utt(idx, osmid, 4)

for line in tqdm(
    open(os.path.join(resource_folder, "location_near_road.demo")),
    desc="generate data",
):
    line = line.strip("\n")
    idx, osmid = line.split("\t")
