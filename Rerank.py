    ...
    for i, datas in enumerate(
        ...
        for bs in range(len(datas["query"])):
            for did in range(len(datas["docs"])):
                ...
                if use_query_gis:
                    query_gis_input_ids.append(
                        datas["query_gis_input_ids"][0][bs]
                    )
...
def evaluation(
    ...
    for datas in tqdm(data_loader):
        for did in range(len(datas["docs"])):
            ...
            if use_query_gis:
                ...
                query_gis_rel_type_ids.append(
                    datas["query_gis_rel_type_ids"][0]
                )
                query_gis_absolute_position_ids.append(
        ...
        if len(text) >= batch_size:
            ...
            if use_query_gis:
                gis_embeds = gis_output.last_hidden_state + model.gis_type(
                    torch.LongTensor([0]).to(gis.input_ids.device)
                )
            else:
            ...
            if use_query_gis:
                ...
                query_gis_embeds = (
                    query_gis_output.last_hidden_state
                    + model.gis_type(
                        torch.LongTensor([1]).to(gis.input_ids.device)
                    )
                )
            ...
            if use_query_gis:
                merge_emb = torch.cat(
                    [
                        embedding_output,
                        model.gis2text(query_gis_embeds),
                        model.gis2text(gis_embeds),
                    ],
                    dim=1,
                )
                ...
                merge_attention = torch.cat(
                    [text_input.attention_mask, gis.attention_mask], dim=-1
                )
            ...
            for gid, gmax in zip(group_ids, gold_max):
                ...
                for pred_relevance, num in zip(
                    ...
                    pred_relevance.append(
                        [float(logits[prev + topkid]) for topkid in topk_ids]
                    )
                if topk_ids[0] < gmax:
                    top1acc += 1
                    mrr1 += 1
                ...
                if output_dir is not None:
                    ...
                    for printid in topk_ids:
                        ...
                        doc.append(
                            ...
                            + ">>"
                            + d
                            + ":{:.2f}".format(logits[printid + prev] * 100)
                        )
                        dpos += 1
                    my_gold = text[prev].split("[SEP]")[1] + ":{:.2f}".format(
                        logits[prev] * 100
                    )
                    out.write(
                        str(hit_top1)
    ...
    if len(text) > 0:
        ...
        if use_query_gis:
            ...
            query_gis_embeds = (
                query_gis_output.last_hidden_state
                + model.gis_type(torch.LongTensor([1]).to(gis.input_ids.device))
            )
            query_gis_atts = query_gis.attention_mask
    ...
    others = {
        ...
        "ndcg@1": float(ndcg_score(true_relevance1, pred_relevance1, k=1)),
        "ndcg@3": float(ndcg_score(true_relevance3, pred_relevance3, k=3)),
        "ndcg@5": float(ndcg_score(true_relevance5, pred_relevance5, k=5)),
        "mrr@1": mrr1 / total,
        "mrr@3": mrr3 / total,
        "mrr@5": mrr5 / total,
    }
