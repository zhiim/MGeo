    ...
    def __init__(
        ...
        if self.warmup_t:
            t_v = (
                ...
                else self._get_lr(self.warmup_t)
            )
            self.warmup_steps = [
                (v - warmup_lr_init) / self.warmup_t for v in t_v
            ]
    ...
    def _get_lr(self, t):
        if t < self.warmup_t:
            ...
            if self.t_mul != 1:
                i = math.floor(
                    math.log(
                        1 - t / self.t_initial * (1 - self.t_mul), self.t_mul
                    )
                )
                t_i = self.t_mul**i * self.t_initial
                t_curr = (
                    t - (1 - self.t_mul**i) / (1 - self.t_mul) * self.t_initial
                )
            else:
                i = t // self.t_initial
                t_i = self.t_initial
            ...
            if self.cycle_limit == 0 or (
                self.cycle_limit > 0 and i < self.cycle_limit
            ):
