...
def create_dataset(dataset, config):
    if dataset == "pretrain_gis" or dataset == "pretrain_mm":
        ...
        train_dataset = rerank_train_dataset(
            config["train_file"],
            use_query_gis=config.get("use_query_gis", False),
        )
        val_dataset = rerank_train_dataset(
            config["val_file"], use_query_gis=config.get("use_query_gis", False)
        )
        test_dataset = rerank_train_dataset(
            config["test_file"],
            use_query_gis=config.get("use_query_gis", False),
        )
