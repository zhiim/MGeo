...
class GisUtt:
    ...
    def update(
        ...
        gis_rel_type_ids = [
            [self.pad_token_id] + json.loads(f) for f in gis_rel_type_ids
        ]
        gis_absolute_position_ids = [
            [[self.pad_token_id] * 4] + json.loads(f)
        ...
        self.attention_mask = torch.tensor(
            [[1] * len(f) + [0] * (max_length - len(f)) for f in gis_input_ids],
            dtype=torch.long,
        ).to(self.device)
...
class rerank_train_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.use_query_gis:
            ...
            datas["query_gis_absolute_position_ids"].append(
                json.dumps(absolute_position)
            )
            datas["query_gis_relative_position_ids"].append(
                json.dumps(relative_position)
            )
        ...
        for item in info["positive_passages"] + info["negative_passages"]:
            ...
            datas["gis_absolute_position_ids"].append(
                json.dumps(absolute_position)
            )
            datas["gis_relative_position_ids"].append(
                json.dumps(relative_position)
            )
