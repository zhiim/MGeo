...
class MGeo(nn.Module):
    ...
    def forward(self, query, doc, gis, query_gis, bs, docnum):
        if query_gis is not None:
            query_gis_output = self.gis_encoder(
            ...
            query_merge_emb = torch.cat(
                [query_embedding_output, self.gis2text(query_gis_embeds)], dim=1
            )
            query_merge_attention = torch.cat(
                [query.attention_mask, query_gis.attention_mask], dim=-1
            )
        ...
        embedding_output = self.text_encoder.embeddings(input_ids=doc.input_ids)
        merge_emb = torch.cat(
            [embedding_output, self.gis2text(gis_embeds)], dim=1
        )
        merge_attention = torch.cat(
            [doc.attention_mask, gis.attention_mask], dim=-1
        )
