...
class MGeo(nn.Module):
    ...
    def forward(self, text, gis, query_gis, bs, docnum):
        ...
        if query_gis is not None:
            gis_embeds = gis_output.last_hidden_state + self.gis_type(
        ...
        if query_gis is not None:
            ...
            query_gis_embeds = (
                ...
                + self.gis_type(torch.LongTensor([1]).to(gis.input_ids.device))
            )
        ...
        if query_gis is not None:
            merge_emb = torch.cat(
                [
                    embedding_output,
                    self.gis2text(query_gis_embeds),
                    self.gis2text(gis_embeds),
                ],
                dim=1,
            )
