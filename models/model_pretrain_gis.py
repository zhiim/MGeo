...
from models.xbert import BertConfig, BertForGisMaskedLM
...
class MGeo(nn.Module):
    ...
    def forward(self, gis, text, lxlys):
        ...
        gis_token_type_ids, gis_token_type_ids_label = self.mask_gis(
            gis_token_type_ids,
            self.gis_encoder.config.type_vocab_size,
            gis.device,
            targets=gis_token_type_ids_label,
            probability_matrix=gis_probability_matrix,
        )
        gis_rel_type_ids, gis_rel_type_ids_label = self.mask_gis(
            gis_rel_type_ids,
            self.gis_encoder.config.rel_type_vocab_size,
            gis.device,
            targets=gis_rel_type_ids_label,
            probability_matrix=gis_probability_matrix,
        )
        (
            gis_absolute_position_ids[:, :, 0],
            gis_absolute_position_ids_label[:, :, 0],
        ) = self.mask_gis(
            gis_absolute_position_ids[:, :, 0],
            self.gis_encoder.config.absolute_x_vocab_size,
            gis.device,
            targets=gis_absolute_position_ids_label[:, :, 0],
            probability_matrix=gis_probability_matrix,
        )
        (
            gis_absolute_position_ids[:, :, 2],
            gis_absolute_position_ids_label[:, :, 2],
        ) = self.mask_gis(
            gis_absolute_position_ids[:, :, 2],
            self.gis_encoder.config.absolute_x_vocab_size,
            gis.device,
            targets=gis_absolute_position_ids_label[:, :, 2],
            probability_matrix=gis_probability_matrix,
        )
        (
            gis_absolute_position_ids[:, :, 1],
            gis_absolute_position_ids_label[:, :, 1],
        ) = self.mask_gis(
            gis_absolute_position_ids[:, :, 1],
            self.gis_encoder.config.absolute_y_vocab_size,
            gis.device,
            targets=gis_absolute_position_ids_label[:, :, 1],
            probability_matrix=gis_probability_matrix,
        )
        (
            gis_absolute_position_ids[:, :, 3],
            gis_absolute_position_ids_label[:, :, 3],
        ) = self.mask_gis(
            gis_absolute_position_ids[:, :, 3],
            self.gis_encoder.config.absolute_y_vocab_size,
            gis.device,
            targets=gis_absolute_position_ids_label[:, :, 3],
            probability_matrix=gis_probability_matrix,
        )
        (
            gis_relative_position_ids[:, :, 0],
            gis_relative_position_ids_label[:, :, 0],
        ) = self.mask_gis(
            gis_relative_position_ids[:, :, 0],
            self.gis_encoder.config.relative_x_vocab_size,
            gis.device,
            targets=gis_relative_position_ids_label[:, :, 0],
            probability_matrix=gis_probability_matrix,
        )
        (
            gis_relative_position_ids[:, :, 2],
            gis_relative_position_ids_label[:, :, 2],
        ) = self.mask_gis(
            gis_relative_position_ids[:, :, 2],
            self.gis_encoder.config.relative_x_vocab_size,
            gis.device,
            targets=gis_relative_position_ids_label[:, :, 2],
            probability_matrix=gis_probability_matrix,
        )
        (
            gis_relative_position_ids[:, :, 1],
            gis_relative_position_ids_label[:, :, 1],
        ) = self.mask_gis(
            gis_relative_position_ids[:, :, 1],
            self.gis_encoder.config.relative_y_vocab_size,
            gis.device,
            targets=gis_relative_position_ids_label[:, :, 1],
            probability_matrix=gis_probability_matrix,
        )
        (
            gis_relative_position_ids[:, :, 3],
            gis_relative_position_ids_label[:, :, 3],
        ) = self.mask_gis(
            gis_relative_position_ids[:, :, 3],
            self.gis_encoder.config.relative_y_vocab_size,
            gis.device,
            targets=gis_relative_position_ids_label[:, :, 3],
            probability_matrix=gis_probability_matrix,
        )
        ...
        gis_return_dic = self.gis_encoder(
            ...
            labels=gis_labels,
            mode="text",
            token_type_ids_label=gis_token_type_ids_label,
            rel_type_ids_label=gis_rel_type_ids_label,
            absolute_position_ids_label=gis_absolute_position_ids_label,
            relative_position_ids_label=gis_relative_position_ids_label,
        )
        ...
        n = len(gis_feat)
        raw_sim_matrix = (
            torch.matmul(gis_feat, gis_feat.t())
            .masked_select(~torch.eye(n, dtype=bool).to(gis.device))
            .view(n, n - 1)
        )
        sim_matrix = F.log_softmax(raw_sim_matrix, dim=1)
        raw_dist_matrix = (
            torch.cdist(lxlys, lxlys, p=2)
            .masked_select(~torch.eye(n, dtype=bool).to(gis.device))
            .view(n, n - 1)
        )
        dist_mean = (
            torch.mean(raw_dist_matrix, dim=1).view(n, 1).expand(n, n - 1)
        )
