...
def train(
    ...
    for i, datas in enumerate(
        ...
        (
            query_gis_input_ids,
            query_gis_token_type_ids,
            query_gis_rel_type_ids,
            query_gis_absolute_position_ids,
            query_gis_relative_position_ids,
        ) = ([], [], [], [], [])
        for bs in range(len(datas["query"])):
            ...
            for did in range(len(datas["docs"])):
                ...
                gis_absolute_position_ids.append(
                    datas["gis_absolute_position_ids"][did][bs]
                )
                gis_relative_position_ids.append(
                    datas["gis_relative_position_ids"][did][bs]
                )
            if use_query_gis:
                query_gis_input_ids.append(datas["query_gis_input_ids"][0][bs])
                query_gis_token_type_ids.append(
                    datas["query_gis_token_type_ids"][0][bs]
                )
                query_gis_rel_type_ids.append(
                    datas["query_gis_rel_type_ids"][0][bs]
                )
                query_gis_absolute_position_ids.append(
                    datas["query_gis_absolute_position_ids"][0][bs]
                )
                query_gis_relative_position_ids.append(
                    datas["query_gis_relative_position_ids"][0][bs]
                )
        ...
        query_input = tokenizer(
            query, padding="longest", return_tensors="pt"
        ).to(device)
        doc_input = tokenizer(doc, padding="longest", return_tensors="pt").to(
            device
        )
...
def evaluation(
    ...
    for datas in tqdm(data_loader):
        ...
        for did in range(len(datas["docs"])):
            ...
            gis_absolute_position_ids.append(
                datas["gis_absolute_position_ids"][did]
            )
            gis_relative_position_ids.append(
                datas["gis_relative_position_ids"][did]
            )
        if use_query_gis:
            query_gis_input_ids.append(datas["query_gis_input_ids"][0])
            query_gis_token_type_ids.append(
                datas["query_gis_token_type_ids"][0]
            )
            query_gis_rel_type_ids.append(datas["query_gis_rel_type_ids"][0])
            query_gis_absolute_position_ids.append(
                datas["query_gis_absolute_position_ids"][0]
            )
            query_gis_relative_position_ids.append(
                datas["query_gis_relative_position_ids"][0]
            )
        ...
        if len(doc) >= batch_size:
            ...
            if use_query_gis:
                ...
                query_gis_output = model.gis_encoder(
                    input_ids=query_gis.input_ids,
                ...
                query_merge_emb = torch.cat(
                    [query_embedding_output, model.gis2text(query_gis_embeds)],
                    dim=1,
                )
                query_merge_attention = torch.cat(
                    [query_input.attention_mask, query_gis.attention_mask],
                    dim=-1,
                )

                query_output = model.text_encoder(
                    attention_mask=query_merge_attention,
                    encoder_embeds=query_merge_emb,
                    return_dict=True,
                    mode="text",
                )
            else:
            ...
            doc_input = tokenizer(
                doc, padding="longest", return_tensors="pt"
            ).to(device)
            embedding_output = model.text_encoder.embeddings(
                input_ids=doc_input.input_ids
            )
            merge_emb = torch.cat(
                [embedding_output, model.gis2text(gis_embeds)], dim=1
            )
            merge_attention = torch.cat(
                [doc_input.attention_mask, gis.attention_mask], dim=-1
            )
            ...
            for gid, gmax in zip(group_ids, gold_max):
                ...
                cur_logits = (
                    query_feat[qid]
                    .view(1, model.embed_dim)
                    .matmul(
                        doc_feat[prev:gid]
                        .view(gid - prev, model.embed_dim)
                        .transpose(0, 1)
                    )
                    .view(-1)
                )
                ...
                for true_relevance, num in zip(
                    [true_relevance1, true_relevance3, true_relevance5],
                    [1, 3, 5],
                ):
                    true_relevance.append(
                        [1 if topkid < gmax else 0 for topkid in topk_ids]
                    )
                for pred_relevance, num in zip(
                    [pred_relevance1, pred_relevance3, pred_relevance5],
                    [1, 3, 5],
                ):
                    pred_relevance.append(
                        [float(cur_logits[topkid]) for topkid in topk_ids]
                    )
                if topk_ids[0] < gmax:
                    top1acc += 1
                    mrr1 += 1
                ...
                if output_dir is not None:
                    ...
                    for printid in topk_ids:
                        ...
                        mydoc.append(
                            ...
                            + ">>"
                            + d
                            + ":{:.2f}".format(cur_logits[printid] * 100)
                        )
                        dpos += 1
    ...
    if len(doc) > 0:
        ...
        if use_query_gis:
            ...
            query_merge_attention = torch.cat(
                [query_input.attention_mask, query_gis.attention_mask], dim=-1
            )
        ...
        for gid, gmax in zip(group_ids, gold_max):
            ...
            topk_ids = topk_ids.tolist()
            for true_relevance, num in zip(
                [true_relevance1, true_relevance3, true_relevance5], [1, 3, 5]
            ):
                true_relevance.append(
                    [1 if topkid < gmax else 0 for topkid in topk_ids]
                )
            for pred_relevance, num in zip(
                [pred_relevance1, pred_relevance3, pred_relevance5], [1, 3, 5]
            ):
                pred_relevance.append(
                    [float(cur_logits[topkid]) for topkid in topk_ids]
                )
    ...
    others = {
        "recall@5": top5acc / total,
        "ndcg@1": float(ndcg_score(true_relevance1, pred_relevance1)),
        "ndcg@3": float(ndcg_score(true_relevance3, pred_relevance3)),
        "ndcg@5": float(ndcg_score(true_relevance5, pred_relevance5)),
        "mrr@1": mrr1 / total,
        "mrr@3": mrr3 / total,
        "mrr@5": mrr5 / total,
    }
...
def main(args, config):
    ...
    for epoch in range(0, max_epoch):
        ...
        val_top1acc, val_top3acc, val_others = evaluation(
            model_without_ddp,
            val_loader,
            tokenizer,
            device,
            ...
            output_name=f"dev-{epoch}",
        )
        test_top1acc, test_top3acc, test_others = evaluation(
            model_without_ddp,
            test_loader,
            tokenizer,
            device,
